...
class AdvancedNetworkImage extends ImageProvider<AdvancedNetworkImage> {
  ...
  Future<ui.Codec> _loadAsync(AdvancedNetworkImage key) async {
    ...
    /// Minimum image.
    Uint8List _minimumImage = Uint8List.fromList([
      137,
      80,
      78,
      71,
    ...
    if (useMemoryCache && _imageMemoryCache != null && _imageMemoryCache.containsKey(uId)) {
      if (useDiskCache) _loadFromDiskCache(key, uId);
      return await ui.instantiateImageCodec(_imageMemoryCache[uId]);
    }
    try {
      if (useDiskCache) return await ui.instantiateImageCodec(await _loadFromDiskCache(key, uId));
    } catch (_) {}
          ...
          if (_diskCacheInfo.containsKey(uId)) {
            if (_diskCacheInfo[uId].length > 0) {
              Map<String, String> _responseHeaders =
                  (await http.head(url, headers: header).timeout(const Duration(milliseconds: 500))).headers;
  ...
  Future<Map> _loadFromRemote(String url, Map<String, String> header, int retryLimit, Duration retryDuration) async {
    /// Retry mechanism.
  ...
  String _uid(String str) => md5.convert(utf8.encode(str)).toString().toLowerCase().substring(0, 9);
