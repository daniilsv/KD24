...
class _TransitionToImageState extends State<TransitionToImage> with TickerProviderStateMixin {
  ...
  @override
  initState() {
    _controller = new AnimationController(vsync: this, duration: widget.duration)..addListener(() => setState(() {}));
    _fadeTween = widget.tween;
    _slideTween = widget.tween;
    if (_fadeTween == null) _fadeTween = new Tween(begin: 0.0, end: 1.0);
    if (_slideTween == null) _slideTween = new Tween(begin: const Offset(0.0, -1.0), end: Offset.zero);
    _reloadListeners.forEach((listener) {
